...
namespace MRCMS.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> SeedTestAuditLogs(int count = 10)
        {
            if (!User.HasClaim("IsSuperAdmin", "true"))
            {
                return Json(new { success = false, message = "Only Super Admins can seed test data." });
            }
            ...
            try
            {
                ...
                {
                    "User", "Role", "Menu", "Blog", "Category", "Tag"
                };
                ...
                var sampleUsers = new[]
                {
                    "admin@example.com", "john.doe@example.com", "jane.smith@example.com", "System"
                };

                var random = new Random();

                // Create test audit logs (default 10, max 100)
                var logCount = Math.Min(count, 100);
                
                for (int i = 0; i < logCount; i++)
                {
                    var entityName = sampleEntities[random.Next(sampleEntities.Length)];
                    var action = sampleActions[random.Next(sampleActions.Length)];
                    var userName = sampleUsers[random.Next(sampleUsers.Length)];
                    var timestamp = DateTime.UtcNow.AddDays(-random.Next(0, 30)).AddHours(-random.Next(0, 24)).AddMinutes(-random.Next(0, 60));

                    var auditLog = new MRCMS.Core.Models.Entities.AuditLog
                    {
                        EntityName = entityName,
                        EntityId = Guid.NewGuid(),
                        Action = action,
                        VersionNumber = random.Next(1, 10),
                        UserName = userName,
                        UserId = userName == "System" ? null : Guid.NewGuid(),
                        Timestamp = timestamp,
                        IpAddress = $"192.168.1.{random.Next(1, 255)}",
                        UserAgent = "Test-Seeder/1.0",
                        ChangedProperties = action == "Modified" ? GenerateChangedProperties(entityName, random) : null,
                        OldValues = action == "Modified" || action == "Deleted" ? GenerateOldValues(entityName, random) : null,
                        NewValues = action == "Modified" || action == "Added" ? GenerateNewValues(entityName, random) : null
                    };
                ...
                return Json(new { success = true, message = $"{logCount} test audit logs have been created successfully!" });
            }
            catch (Exception ex)
            {
                _logger.LogError("Error seeding test audit logs", ex);
                return Json(new { success = false, message = $"Error: {ex.Message}" });
            }
        }

        private string GenerateChangedProperties(string entityName, Random random)
        {
            var properties = entityName switch
            {
                "User" => new[] { "FirstName", "LastName", "Email", "PhoneNumber", "IsActive" },
                "Role" => new[] { "Name", "Description", "IsActive" },
                "Menu" => new[] { "Title", "Url", "IconClass", "IsActive", "Order" },
                "Blog" => new[] { "Title", "Content", "Summary", "IsPublished", "PublishedAt" },
                "Category" => new[] { "Name", "Description", "IsActive" },
                "Tag" => new[] { "Name", "IsActive" },
                _ => new[] { "Name", "IsActive" }
            };

            var numChanged = random.Next(1, Math.Min(4, properties.Length));
            var changedProps = properties.OrderBy(x => random.Next()).Take(numChanged);
            return string.Join(", ", changedProps);
        }

        private string GenerateOldValues(string entityName, Random random)
        {
            object values = entityName switch
            {
                "User" => new { FirstName = "John", LastName = "Doe", Email = "john.old@example.com", IsActive = true },
                "Role" => new { Name = "Old Role", Description = "Old description", IsActive = true },
                "Menu" => new { Title = "Old Menu", Url = "/old-url", IsActive = true },
                "Blog" => new { Title = "Old Blog Title", IsPublished = false },
                _ => new { Name = "Old Name", IsActive = true }
            };

            return System.Text.Json.JsonSerializer.Serialize(values);
        }

        private string GenerateNewValues(string entityName, Random random)
        {
            object values = entityName switch
            {
                "User" => new { FirstName = "Jane", LastName = "Smith", Email = "jane.new@example.com", IsActive = true },
                "Role" => new { Name = "New Role", Description = "New description", IsActive = true },
                "Menu" => new { Title = "New Menu", Url = "/new-url", IsActive = true },
                "Blog" => new { Title = "New Blog Title", IsPublished = true },
                _ => new { Name = "New Name", IsActive = true }
            };
